...
db.pedidos.find({"Fnacimiento": {$gte: new Date(1963, 1, 1), $lt: new Date(1964, 1, 1)}}).pretty()
...
db.pedidos.find({"Nombre": /^c/i}, {"_id": 0, "id_cliente": 1, "Nombre": 1, "Direccion": 1, "Localidad": 1, "Fnacimiento": 1,}).pretty()

// 10. Visualiza los datos personales de los clientes (excluyendo _id). Limita los documentos a 4
db.pedidos.find({}, {"Pedidos": 0, "_id": 0}).limit(4).pretty()

// 11. Ídem anterior pero ordenando los documentos por Localidad (ascendente) e id_cliente (descendente)
db.pedidos.find({}, {"Pedidos": 0, "_id": 0}).sort({"Localidad": 1, "id_cliente": -1}).limit(4).pretty()